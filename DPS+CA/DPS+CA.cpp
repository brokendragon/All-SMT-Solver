...
#include <fstream>
#include <direct.h>
#include "E:\project\Program_Analysis\z3_program\CNS\z3-4.3.2-x64-win\include\z3++.h" //change to your path

using namespace z3;

#define TIME_OUT 10800 // in seconds
#define MAX_SOLUTION 1000000 //when solution outnumbers 1000000, file system runs very slow
#define MEMORY_OUT 10240 //in MB
//#define VERBOSE
#define BIT_WIDTH 3
#define PARALLEL 1 //create worker threads
#define PREVIOUS_TASK_POOL_SIZE 100 //the length of preTaskPool

unsigned readTaskNum = 0;

Z3_string fname = "E:\\project\\Program_Analysis\\z3_program\\CNS\\benchmark\\adjust\\z3test2.smt2";//path of benchmark file, change to your path
...
{
	time(&end_t);
	std::cout << "time cost: " << (end_t - start_t) << " (s), peak memory cost: " << memoryCost / (1024 * 1024) << "(MB)\n";
	WaitForSingleObject(eNumber, INFINITE);
...
VOID WINAPI countDown()
{
	...
	while (1) {
		if (i * 600 >= TIME_OUT || SOLUTION_NUM >= MAX_SOLUTION)
			break;//output statistics every minute
		i++;
		Sleep(1000 * 600);
		outputStatistics();
	}
...
void produceSolvers()
{
	try {
		for (int i = 0; i < PARALLEL; i++)
		{
			...
			Z3_ast query;
			query = Z3_parse_smtlib2_file(sv->ctx(), fname, 0, 0, 0, 0, 0, 0);
