...
unsigned MEMORY_OUT = 1000; //in MB
//#define VERBOSE
#define BIT_WIDTH 7
unsigned PARALLEL = 4; //create worker threads
unsigned PREVIOUS_TASK_POOL_SIZE = 100; //the length of preTaskPool

unsigned readTaskNum = 0;

std::string smtFile;//path of benchmark file
SIZE_T memoryCost = 0;
unsigned TASK_ID = 0;
...
context **ctxPool = (context**)malloc(PARALLEL * sizeof(context*));
solver **reasonResourcePool = (solver**)malloc(PARALLEL * sizeof(solver*));//for incremental solving
...
void outputToFile(std::map<std::string, VARIBLE_VALUE> *parsedSolution, unsigned taskID)
{
	...
	std::string fname;
	if (parsedSolution == NULL) //indicates default solution
	{
		parsedSolution = &defaultSolution;
		fname = std::string(path) + "\\defaultSolution.txt";
	}
	else
		fname= std::string(path) + "\\solutions\\solution_" + ss.str() + std::string(".txt");
...
void readSolution(unsigned taskID, unsigned id, std::vector<VARIBLE_VALUE> *vv)
{
	...
	GetCurrentDirectory(MAX_PATH, (LPTSTR)path);
	std::string fname;
	if ((taskID == 0) && (id == 0)) //read default solution
		fname = std::string(path) + "\\defaultSolution.txt";
	else
		fname = std::string(path) + "\\solutions\\solution_" + ss.str() + std::string(".txt");
...
*/
solver* assembleCons(TASK *tsk, int threadSeq)
{
	try {
		solver *sv = reasonResourcePool[threadSeq];
...
void spawnTasks(solver *sv, TASK* tsk, std::map<std::string, VARIBLE_VALUE> *parsedModel)
{
	try {
		...
		TASK *n_tsk = NULL;
		if ((var_type == "bool") && (tsk->f_range != -1))
		{
...
{
	try{	
		solver *sv = assembleCons(tsk, threadSeq);
		if (sv->check() == sat)//sat?
		{
		...
		sv->pop();//restore to original constraints
		//release the space for completed task
	}
	catch (exception & ex)
	{
		std::cout << "unexpected error: " << ex << "\n";
...
{
	//std::cout "peak memory cost: " << memoryCost / (1024 * 1024) << "(MB)\n";
	WaitForSingleObject(eNumber, INFINITE);
...
void DumpTasks()
{
	...
	if (!fs.is_open())
	{
		std::cout << "failed to create endID file\n";
		exit(0);
	}
	fs << taskPool[taskPool.size() - 1]->taskID;
	fs.close();
	taskPool.clear();
...
/*
alloc solver from heap, and fill reasonResourcePool
*/
void produceSolvers()
{
	try {
		for (int i = 0; i < PARALLEL; i++)
		{
			...
			ctxPool[i] = ctx;
			solver *sv = new solver(*ctxPool[i]);
			/*expr x = sv->ctx().bv_const("x", BIT_WIDTH),
				y = sv->ctx().bv_const("y", BIT_WIDTH),
				z = sv->ctx().bv_const("z", BIT_WIDTH);
			expr ex = (x > 0) && (x < 31) && (y > 0) && (y < 31) && (z > 0) && (z < 31);
			sv->add(ex);*/

			Z3_ast query;
			query = Z3_parse_smtlib2_file(sv->ctx(), smtFile.c_str(), 0, 0, 0, 0, 0, 0);
			sv->add(to_expr(sv->ctx(), query));
			...
			if (sv->check() == sat)//sat?
			{
				model m = sv->get_model();
				if (m)//has a model
					reasonResourcePool[i] = sv;
				else
				{
					std::cout << "error in fillSolverPool: cannot find a model for original constraint!" << "\n";
					exit(0);
				}
			}
			else
			{
				std::cout << "error in fillSolverPool: original constraint is unsat!" << "\n";
				exit(0);
			}
		}
	}
...
void getConfig()
{
	char path[MAX_PATH] = { 0 };
	GetCurrentDirectory(MAX_PATH, (LPTSTR)path);
	std::string fname = std::string(path) + "\\configure" + std::string(".txt");
	std::ifstream fs(fname);
	if (!fs.is_open())
	{
		std::cout << "failed to open mode file\n";
		exit(0);
	}
	fs >> smtFile >> mode >> MEMORY_OUT >> PARALLEL >> PREVIOUS_TASK_POOL_SIZE >> endID;
	fs.close();
}

/*
read default solution from file, when the job is continuing from previous one
*/
void readDefaultSolution()
{
	std::vector<_VARIBLE_VALUE> vv;
	readSolution(0, 0, &vv);
	for (unsigned i = 0; i < vv.size(); i++)
		defaultSolution.insert(std::make_pair(vv[i].name, vv[i]));
}
...
void main()
{
	...
	{
		readDefaultSolution(); //read default solution from file
		getTaskFileOffset();
		openTaskFile();
		TASK_ID = endID + 1;		
	}
	...
	setAllEvents();	
	HANDLE *hThread = (HANDLE*)malloc(PARALLEL*sizeof(HANDLE));
	produceSolvers();
	if (!mode) //write default solution into disk
		outputToFile(NULL, 0);
	for (int i = 0; i < PARALLEL; i++)
	{
		DWORD threadID;
	...
	outputStatistics();
	exit(0xff); //indicate task has completed
}